    ...
    return (
        <div className="space-y-4">
            <div>
                ...
                <p className="text-xs text-blue-200 mt-0.5">실시간 업데이트 (3초마다)</p>
            </div>

            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {Array.isArray(queues) && queues.map((queue: any) => (
                    <div key={queue.name} className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl p-3">
                        ...
                        <div className="grid grid-cols-2 gap-1 text-xs mb-2">
                            <div className="flex justify-between"><span className="text-blue-300/70">대기</span><span className="text-amber-400">{queue.waiting}</span></div>
                            <div className="flex justify-between"><span className="text-blue-300/70">처리중</span><span className="text-blue-400">{queue.active}</span></div>
                            <div className="flex justify-between"><span className="text-blue-300/70">완료</span><span className="text-emerald-400">{queue.completed}</span></div>
                            <div className="flex justify-between"><span className="text-blue-300/70">실패</span><span className="text-red-400">{queue.failed}</span></div>
                        </div>

                        <div className="space-y-1">
                            <button onClick={() => setSelectedQueue(queue.name)} disabled={queue.failed === 0}
                                className="w-full px-2 py-1 text-xs bg-blue-600/20 text-blue-400 rounded-lg hover:bg-blue-600/30 disabled:opacity-50">
                                실패 작업 ({queue.failed})
                            </button>
                            <button onClick={() => handleClearCompleted(queue.name)} disabled={queue.completed === 0}
                                className="w-full px-2 py-1 text-xs bg-white/5 text-blue-200 rounded-lg hover:bg-white/10 disabled:opacity-50">
                                완료 정리
                            </button>
                        </div>
                    </div>
            ...
            {selectedQueue && failedJobs && failedJobs.length > 0 && (
                <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl overflow-hidden">
                    <div className="px-3 py-2 border-b border-white/10 flex justify-between items-center">
                        ...
                        <button onClick={() => setSelectedQueue(null)} className="text-blue-300 hover:text-white text-xs">✕</button>
                    </div>
                    <div className="p-3 space-y-2 max-h-60 overflow-y-auto">
                        {failedJobs.map((job: any) => (
                            <div key={job.id} className="bg-white/5 rounded-lg p-2 text-xs">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <span className="text-white">#{job.id}</span>
                                        <span className="ml-2 text-blue-300/70">시도: {job.attemptsMade}회</span>
                                    </div>
