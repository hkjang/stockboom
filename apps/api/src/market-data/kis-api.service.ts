...
export class KisApiService {
     ...
     */
    private async refreshToken(retryCount: number = 0): Promise<string> {
        const MAX_RETRIES = 3;
     ...
     */
    async getQuote(symbol: string): Promise<KISQuote> {
     ...
     * Place an order
     */
    async placeOrder(orderRequest: KISOrderRequest): Promise<KISOrderResponse> {
        const token = await this.getAccessToken();
        const appKey = this.configService.get('KIS_APP_KEY');
        const accountNumber = this.configService.get('KIS_ACCOUNT_NUMBER');
        const isMockMode = this.configService.get('KIS_MOCK_MODE') === 'true';
     ...
     */
    async getCandles(symbol: string, timeframe: string, count: number = 100): Promise<any[]> {
        const token = await this.getAccessToken();
        const appKey = this.configService.get('KIS_APP_KEY');
