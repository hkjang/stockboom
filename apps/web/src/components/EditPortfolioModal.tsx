...
export default function EditPortfolioModal({ isOpen, onClose, onSuccess, portfolio }: EditPortfolioModalProps) {
    ...
    useEffect(() => {
        if (portfolio) {
            ...
            setAutoTrade(portfolio.autoTrade || false);
        }
    }, [portfolio]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setError(data.message || '포트폴리오 수정에 실패했습니다.');
            }
        } catch (err) {
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-800 rounded-xl max-w-md w-full border border-white/10">
                <div className="flex items-center justify-between p-6 border-b border-white/10">
                    <h3 className="text-xl font-bold text-white">포트폴리오 수정</h3>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        />
                        <label htmlFor="autoTrade" className="text-blue-200 text-sm">
                            자동매매 활성화
                        </label>
                    </div>
